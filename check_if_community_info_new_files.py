...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Python program to check new Community Info new files from ATTOM Jetstream API.',
                                     add_help=True)
...
def check_if_community_info_new_files(zip_files):
    ...
    for x in range(len(zip_files)):
        ...
        if str(zip_files[x]).__contains__('community-info'):
            log.info('New data for Community Info is available!')
            new_files = True
            return new_files
...
if __name__ == '__main__':
    ...
    if last_folder_date < files_upload_date:
        ...
        new_community_info_flag = check_if_community_info_new_files(files_names)

        if new_community_info_flag:
            print('New Community Info: TRUE')
        else:
            print('New Community Info: FALSE')
